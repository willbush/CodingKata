...
-- | This takes a predicate that is used to filter the error messages by error
...
-- | This takes a list of "words" (i.e. what you get if you split a string on
-- white-space) that all belong to a single log message of the type Info and
-- parses it into a Info type.
...
-- | This takes a list of "words" (i.e. what you get if you split a string on
-- white-space) that all belong to a single log message of the type warning and
-- parses it into a Warning type.
...
-- | This takes a list of "words" (i.e. what you get if you split a string on
-- white-space) that all belong to a single log message of the type error and
-- parses it into a Error type.
...
-- | This takes a predicate that is used to filter the error messages by error
-- level and a LogMessage list. the returned list is sorted by time stamp and
-- contains only the filtered error message string.
...
-- | This builds a unbalanced binary search tree. How balanced the tree is largely
-- depends on the first insertion and how close its time stamp is to the median
-- of all time stamps. The tree is used to sort the messages by time stamp. As
-- pointed out by the homework exercise, there are better ways to sort and doing
-- it this way is mostly for practice with recursive data structures.
buildTree :: [LogMessage] -> MessageTree
buildTree = foldl' (flip insert) Leaf

-- | This inserts a log message into the message tree. Message tree is a binary
-- search tree with left child time stamp < parent time stamp and right
-- child >= parent time stamp.
insert :: LogMessage -> MessageTree -> MessageTree
insert message tree = case extractTimeStamp message of
  Just time  -> insert' message time tree
  Nothing    -> tree

-- | This helper function takes the extracted time stamp from the log message to
-- needs to be inserted and then extracts the time stamp for the current node in
-- the tree, compares the time stamps and makes the proper recursive call or insertion.
...
-- | This extracts a Maybe TimeStamp from a LogMessage since a LogMessage type
-- can also be Unknown.
...
-- | This takes a message tree, which is an unbalanced binary search tree and
-- performs an in order traversal to build a sorted log message list. However,
-- unlike normal in order traversals, this function traverses the right children
-- first instead of the left. This has the effect of visiting the nodes in order
-- from greatest time stamp to least. Visiting nodes in this order works well
-- because it's easier to cons elements from greatest to least into an empty list
-- to get the resulting list sorted from least to greatest.
-- Otherwise, the list would of needed to be reversed.
...
-- | This takes a predicate (such as (>= 50)) that is used against the error 
-- level of the given log message. If the log message is not of the type 'Error',
